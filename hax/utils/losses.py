...
@partial(jax.jit, static_argnames=['num_rotations', 'hasNegatives', 'resample_size', 'mse_lambda'])
def wasserstein_distance_loss_images(x: jnp.ndarray, x_true: jnp.ndarray, num_rotations: int = 100, hasNegatives: bool = False,
                                     resample_size: int = 64, mse_lambda: float = 10., ctf: jnp.ndarray = None):
    # Wiener fiiltering auxiliary function
    ...
    # Soft circular mask
    Y, X = jnp.ogrid[:resample_size, :resample_size]
    dist_from_center = jnp.sqrt((X - 0.5 * resample_size) ** 2 + (Y - 0.5 * resample_size) ** 2)
    mask = (dist_from_center <= 0.5 * resample_size).astype(jnp.float32)[None, ..., None]
    mask = dm_pix.gaussian_blur(mask, 1.0, kernel_size=5)[..., 0]

    # Wiener filter
    if ctf is not None:
        # Soft circular mask
        Y, X = jnp.ogrid[:x.shape[1], :x.shape[1]]
        dist_from_center = jnp.sqrt((X - 0.5 * x.shape[1]) ** 2 + (Y - 0.5 * x.shape[1]) ** 2)
        mask_binary = (dist_from_center <= 0.5 * x.shape[1]).astype(jnp.float32)[None, ...]

        x_true_no_ctf = wiener2DFilter(x_true, ctf, pad_factor=2)

        # Compute bias
        b_true = x_true_no_ctf.min(axis=(1, 2), keepdims=True, where=(mask_binary == 0.0), initial=0.0)
        b_true = jnp.where(b_true < 0.0, -b_true, 0.0)
        b = (x_true_no_ctf + b_true).mean(axis=(1, 2), keepdims=True, where=(mask_binary == 0.0))
        a = (x_true_no_ctf + b_true).std(axis=(1, 2), keepdims=True, where=(mask_binary == 0.0))

        # Make all values positive in the images
        x_true = x_true + ctfFilter(jnp.ones_like(x_true) * b_true, ctf, pad_factor=2)
        x = x + ctfFilter(jnp.ones_like(x) * b, ctf, pad_factor=2)
        # x = ctfFilter(jnp.ones_like(x) * a, ctf, pad_factor=2) * x + ctfFilter(jnp.ones_like(x) * b, ctf, pad_factor=2)
        # x = x + ctfFilter(jnp.ones_like(x) * b_true, ctf, pad_factor=2)

        # What about CTF²?
        # x_true = ctfFilter(x_true, ctf, pad_factor=2)
        # x = ctfFilter(x, ctf, pad_factor=2)

    # MSE to loss
    # mse = mse_lambda * dm_pix.mse(x[..., None], x_true[..., None]).mean()

    # Make all values positive in the images
    # # b = 1.2 * x_true.min(axis=(1, 2), keepdims=True)
    # b = x_true.min(axis=(1, 2), keepdims=True)
    # b = jnp.where(b < 0.0, -b, 0.0)
    # # x = x + b
    # x_true = x_true + b
    # x = x + x_true.mean(axis=(1, 2), keepdims=True)

    # Resize images (to make loss computation faster)
    x = jax.image.resize(x, (x.shape[0], resample_size, resample_size), method='bilinear')
    x_true = jax.image.resize(x_true, (x_true.shape[0], resample_size, resample_size), method='bilinear')

    # Rotate images
    rotate_batch_over_rotations = jax.vmap(lambda img, rotations: dm_pix.rotate(img, rotations, mode="constant"),
                                           in_axes=(None, 0))
    rotate = jax.vmap(lambda imgs, rotations: rotate_batch_over_rotations(imgs, rotations), in_axes=(0, None))
    # in_plane_rotations = jnp.linspace(0, jnp.pi, num_rotations, endpoint=False)
    key = jax.random.PRNGKey(random.randint(0, 2 ** 32 - 1))
    in_plane_rotations = jax.random.uniform(key, minval=0.0, maxval=jnp.pi, shape=(num_rotations,))
    rotated_x = rotate(x[..., None], in_plane_rotations)[..., 0]
    rotated_x_true = rotate(x_true[..., None], in_plane_rotations)[..., 0]

    # Split in positive and negative
    if hasNegatives:
        rotated_x_pos, rotated_x_neg = nnx.relu(rotated_x), nnx.relu(-rotated_x)
        rotated_x_true_pos, rotated_x_true_neg = nnx.relu(rotated_x_true), nnx.relu(-rotated_x_true)
    else:
        rotated_x_pos = nnx.relu(rotated_x)
        rotated_x_true_pos = nnx.relu(rotated_x_true)

    # Apply soft mask to images
    if hasNegatives:
        rotated_x_pos, rotated_x_neg = rotated_x_pos * mask[None, ...], rotated_x_neg * mask[None, ...]
        rotated_x_true_pos, rotated_x_true_neg = rotated_x_true_pos * mask[None, ...], rotated_x_true_neg * mask[None, ...]
    else:
        rotated_x_pos = rotated_x_pos * mask[None, ...]
        rotated_x_true_pos = rotated_x_true_pos * mask[None, ...]

    # 1D strips
    if hasNegatives:
        strips_x_pos, strips_x_neg = rotated_x_pos.sum(axis=-1), rotated_x_neg.sum(axis=-1)  # (B, N, n)
        strips_x_true_pos, strips_x_true_neg = rotated_x_true_pos.sum(axis=-1), rotated_x_true_neg.sum(axis=-1)  # (B, N, n)
    else:
        strips_x_pos = rotated_x_pos.sum(axis=-1)  # (B, N, n)
        strips_x_true_pos = rotated_x_true_pos.sum(axis=-1)  # (B, N, n)

    # Wassestein distance (based on code: https://github.com/flatironinstitute/Cryo-EM-Heterogeneity-Challenge-1/blob/sliced_w/src/cryo_challenge/map_to_map/sliced_wasserstein/pixel_strip.py#L49)
    def wasserstein_over_one_strip(a, b):
        eps = 1e-8
        a = a / (a.sum(axis=-1, keepdims=True) + eps)  # (B, n)
        b = b / (b.sum(axis=-1, keepdims=True) + eps)  # (B, n)

        # Compute CDFs
        cdf_a = jnp.cumsum(a, axis=-1)  # (B, n)
        cdf_b = jnp.cumsum(b, axis=-1)  # (B, n)

        # Compute pairwise squared L2 distances between CDFs
        diff = cdf_a - cdf_b  # (B, n)

        return (diff ** 2.).sum(axis=-1)  # (B,)
    wasserstein_over_strips = jax.vmap(wasserstein_over_one_strip, in_axes=(1, 1))  # (B, N)

    w = wasserstein_over_strips(strips_x_pos, strips_x_true_pos).mean(axis=0)
    # strips_x_pos = strips_x_pos / (strips_x_pos.sum(axis=-1, keepdims=True) + 1e-8)
    # strips_x_true_pos = strips_x_true_pos / (strips_x_true_pos.sum(axis=-1, keepdims=True) + 1e-8)  # (B, n)
    # fn = jax.vmap(jax.vmap(UnivariateWasserstein(uniform_solver, SqEuclidean())))
    # w = fn(strips_x_pos, strips_x_true_pos)
    w = w.mean()

    # Unbalanced Wasserstein distance (taken from Jax OTT UnivariateWasserstein)
    # strips_x_pos = strips_x_pos / (strips_x_pos.sum(axis=-1, keepdims=True) + 1e-8)
    # strips_x_true_pos = strips_x_true_pos / (strips_x_true_pos.sum(axis=-1, keepdims=True) + 1e-8)
    # geom = pointcloud.PointCloud(strips_x_pos[:, None], strips_x_true_pos[:, None], cost_fn=SqEuclidean())
    # prob = linear_problem.LinearProblem(geom)
    # out = uniform_solver(prob)
    # w = jnp.squeeze(out.ot_costs).mean()

    if hasNegatives:
        w_neg = wasserstein_over_strips(strips_x_neg, strips_x_true_neg).mean(axis=0)
        w = w + w_neg.mean()

        # strips_x_neg = strips_x_neg / (strips_x_neg.sum(axis=-1, keepdims=True) + 1e-8)
        # strips_x_true_neg = strips_x_true_neg / (strips_x_true_neg.sum(axis=-1, keepdims=True) + 1e-8)  # (B, n)
        # fn = jax.vmap(jax.vmap(UnivariateWasserstein(uniform_solver, SqEuclidean())))
        # w = w + fn(strips_x_neg, strips_x_true_neg)

        # strips_x_neg = strips_x_neg / (strips_x_neg.sum(axis=-1, keepdims=True) + 1e-8)
        # strips_x_true_neg = strips_x_true_neg / (strips_x_true_neg.sum(axis=-1, keepdims=True) + 1e-8)
        # geom = pointcloud.PointCloud(strips_x_neg[:, None], strips_x_true_neg[:, None], cost_fn=SqEuclidean())
        # prob = linear_problem.LinearProblem(geom)
        # out = uniform_solver(prob)
        # w = w + jnp.squeeze(out.ot_costs).mean()
